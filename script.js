...
const kidsWithCandies = (candies, extraCandies) => {
    ...
    for (let i = 0; i < candies.length; i++) {
        if (candies[i] > largestCandy) {
            largestCandy = candies[i];
        }
    }
    let outputArray = candies.map((candy) => {
        if (candy + extraCandies >= largestCandy) {
            return true;
        } else {
            return false;
        }
...
console.log(kidsWithCandies([2, 3, 4, 5, 1, 3], 1));


// and another... (best solution!)
const kidsWithCandy = (candies, extraCandies) => {
    const maxCandies = Math.max(...candies)
    return candies.map((candy) => candy + extraCandies >= maxCandies)
};
console.log(kidsWithCandy([2, 3, 4, 5, 1, 3], 1));
